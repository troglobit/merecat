...
# Pass 1/15: request is forwarded and response comes from the backend
echo "Pass 1/15"
curl -s http://localhost:8086/proxy/hello | grep '"path".*"/proxy/hello"'

# Pass 2/15: X-Forwarded-For and X-Real-IP headers are injected.
...
# Pass 3/15: path prefix stripped when backend URL carries a path component
#   GET /v2proxy/hello -> GET /v2/hello forwarded to backend
echo "Pass 3/15"
curl -s http://localhost:8086/v2proxy/hello | grep '"path".*"/v2/hello"'

# Pass 4/15: path prefix stripped when backend URL has a trailing slash only
#   GET /strip/hello -> GET /hello forwarded to backend
echo "Pass 4/15"
curl -s http://localhost:8086/strip/hello | grep '"path".*"/hello"'

# Pass 5/15: proxy-redirect rewrites Location: header in backend response
...
# Pass 6/15: query string is forwarded exactly once
...
# Pass 7/15: query string survives path prefix stripping
...
# Pass 8/15: POST body larger than one read() arrives complete at the backend
...
# Pass 9/15: POST body over the 8 MiB cap is rejected with 413 up front
echo "Pass 9/15"
code=$(head -c 9437184 /dev/zero | curl -s -o /dev/null -w '%{http_code}' \
	--data-binary @- http://localhost:8086/proxy/upload)
test "$code" = "413"

# Pass 10/15: backend that closes without sending anything yields 502
echo "Pass 10/15"
python3 - <<'EOF' &
import socket
srv = socket.socket()
...
# Pass 11/15: malformed proxy-redirect refuses to start instead of
...
# Pass 12/15: backend that resets the connection (closes without reading
...
# Pass 13/15: successful proxied requests show up in the access log with
...
# Pass 14/15: IPv6-only backend, configured as a bracketed literal
echo "Pass 14/15"
python3 - <<'EOF' &
...
# Pass 15/15: proxy-redirect = "default" derives FROM and TO from the rule
#   Rule: pattern "/dredir/**", backend "http://localhost:9090/" (strip)
#   Backend returns: Location: http://localhost:9090/redir/foo/target
#   Merecat rewrites to the relative /dredir/redir/foo/target
echo "Pass 15/15"
loc=$(curl -s -o /dev/null -D - http://localhost:8086/dredir/redir/foo | grep -i '^Location:')
echo "$loc" | grep "Location: /dredir/redir/foo/target"
